    ...
    private func callNotifications()
     {
        ...
        ApiHelper.shared.getNotifications { (success, arrs) in
            if let notifs = arrs
            {
                ...
                var numberConnectTrade = 0
                var numberRequestTrade = 0
               ...
               for item in notifs
               {
                    ...
                    {
                      if item.actType == "requested_cdd"
                      {
                            self.arrIDNotificationRequest.append(item)
                          numberRequestTrade = numberRequestTrade + 1
                      }
                      else if item.actType == "accepted_trade_connect"
                      {
                             self.arrIDNotificationConnection.append(item)
                          numberConnectTrade = numberConnectTrade + 1
                      }
                      else{
                ...
                if UserDefaults.standard.bool(forKey: KEY_NOTIFICATION_SETTING) {
                       ...
                       if APP_DELEGATE.notificationChangeUserName != nil && APP_DELEGATE.notificationChangeProfilePic != nil {
                            self.tabBarController?.tabBar.items![2].badgeValue = "3"
                       }
                       else{
                  ...
                  if numberRequest +  numberConnectiton + numberRequestTrade + numberConnectTrade > 0
                  {
                      self.tabBarController?.tabBar.items![1].badgeValue = "\(numberRequest + numberConnectiton + numberRequestTrade + numberConnectTrade)"
                  }
                  else{
                ...
                if numberRequestTrade > 0
                 {
                      self.subRequest.isHidden = false
                      self.lblNumberRequest.text = "\(numberRequestTrade)"
                 }
                 else{
                      self.subRequest.isHidden = true
                 }
                  if numberConnectTrade > 0
                {
                     self.subTrade.isHidden = false
                     self.lblNumberTrade.text = "\(numberConnectTrade)"
                }
                else{
                     self.subTrade.isHidden = true
                }
            }
            else{
              self.tabBarController?.tabBar.items![1].badgeValue = nil
